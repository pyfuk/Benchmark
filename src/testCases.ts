...
function getTestsBench(test: ItestObject, iterations: number, repeats: number) {
  ...
  logInfo(counters, repeats); // хуета
}
...
function logInfo(counters: IsetCounters, repeats: number) {
  console.log(counters.testTitle);
  console.log(
    "Avg work time: " + (counters.counterTime / repeats).toFixed(2) + " ms"
  );
  console.log(
    "Avg memory used: " +
      (counters.counterMemoryUsage / repeats).toFixed(2) +
      " MB"
  );
  console.log(
    "Avg CPU used: " + (counters.counterCpu / repeats).toFixed(2) + " %"
  );
  console.log(
    "---------------------------------------------------------------"
  );
